...
def criar_tabela_contagem_cliques():
    ...
    try:
        ...
        cursor.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name='click_counts';")
        if cursor.fetchone() is None:  # Se a tabela não existe
            ...
            cursor.execute(
                "INSERT INTO click_counts (event_name, count) VALUES (?, ?)", ('contact_anunciante_click', 0))
            print(
                "Tabela 'click_counts' criada e inicializada com 'contact_anunciante_click'.")
        else:
